...
`nixconf` reads `nixconf.yaml` from the workspace root. The file lists the
repos to manage, grouped into `common`, `hosts`, and `users`:
...
|--------------------------------|--------------------------------------------------------------------|
| `nixconf clone`                | Clone every repo in `nixconf.yaml` into its `path`                 |
...
| `nixconf <anything else>`      | Forwarded to `git -C <repo> <args>` in every selected repo         |
...
| `--config <path>`   | override the nixconf.yaml lookup        |

### Examples

```bash
# clone every repo declared in nixconf.yaml
nixconf clone
...
# git pass-through
nixconf status               # git status in every repo
nixconf --hosts pull         # git pull in every host repo
nixconf -r nixos-common log -5
```
